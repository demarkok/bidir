...
\begin{mathpar}
  ...
  \Infer{$\mathsf{eq}$E$\infers$}
        { \judgevInfer{\Gamma}{v}{\eqtype{A}{w_1}{w_2}} \\
          \judgectype{\Gamma, y : A, q : \eqtype{A}{w_1}{y}}{X} \\
          \judgecCheck{\Gamma}{t}{\subst{\subst{X}{y}{w_1}}{q}{\refl}} }
        { \judgecInfer{\Gamma}{\receq{v}{X}{t}}{\subst{\subst{X}{y}{w_2}}{q}{v}} }
  \and
  ...
  \Infer{$\subt$Eq}
        {
          ...
          \judgeequiv{\Gamma}{w_1}{w_2}{B} \\
        }
        {\judgevsubt{\Gamma}{\eqtype{A}{v_1}{w_1}}{\eqtype{A}{v_2}{w_2}}}
...
\begin{itemize}
\item \ilyam{We don't have kinds, so we cannot form expressions of type * -> *
  Doesn't it mean that System F is not embedded?}
\item \ilyam{Why is it fine not to have
  $X \to \foralltype{x:A}{Y} \subt \foralltype{x:A}{X \to Y}$
  (given $x$ is not free in $X$)? What about the reverse direction? }
