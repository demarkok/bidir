...
We write \emph{term} meaning a computation or a value.
...
\ilyam{Do we reduce the types? If so, we need to add types to the definition of Syntactic normal form.}
...
\begin{definition}[Operational normal form]
  We say that the term $t$ is in the operational
...
\begin{definition}[Syntactic normal form]
  We say that the term $t$ is in the syntactic
  normal form if and only if it is form by the following grammar:

  \begin{mathpar}
    \begin{array}{llcl}
      \mbox{Normal Computations} & t_n & ::= &
      \fun{x:A}{t_n} \bnfalt
      \return{v_n} \bnfalt
      t_a
      \\[1em]
      ...
      \force{v_a} \bnfalt
      t_a v_n \bnfalt
      \recsigma{v_a}{X}{t_n} \bnfalt
      \receq{v_a}{X}{t_n} \bnfalt \\ &&& 
      \letval{x : A}{t_a}{t_n} \bnfalt
      \dletval{x : A}{t_a}{t_n}
      \\[1em]

      \mbox{Normal Values} & v_n & ::= &
      \pair{v_n}{v_n} \bnfalt
      \thunk{t_n} \bnfalt
      \refl \bnfalt
      v_a
      ...
      \mbox{Atomic Values} & v_a & ::= & x
      \\[1em]

    \end{array}
  \end{mathpar}

  Normal terms are not reducible. Atomic terms, in addition, do not cause
  reduction when destructors are applied to them.

\end{definition}

\begin{lemma}[Normal form equivalence]
  A term $t$ is in the operational normal form if and only if $t$ is in the syntactic normal form.
  \ilyam{(assuming types are not reduced)}
\end{lemma}
...
\begin{proof}
  ...
  \item [($\Rightarrow$)] By induction on the term structure.
    \ilyam{This is only true for the well-typed terms!}
...
\begin{definition} [x-neutral form]
  Assume that $x$ is a variable and $t$ is a term. 
  We say that $t$ is $x$-neutral if and only if
  \begin{enumerate*}
...
We define equivalence syntactically,
as such, we do not maintain the typing information throughout the algorithm.

\begin{definition}[Equivalence]  The unification algorithm (and the equivalence relation) is defined
